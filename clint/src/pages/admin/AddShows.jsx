...
const AddShows = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log("Submitting show with payload:", payload);
      ...
      toast.error("An error ccurred. please try again.");
    }
    setAddingShow(false);
  }
    ...
    <>
            ...
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40
             hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
              ...
              {selectedMovie == movie.id && (
                <div className='absolute top-2 right-2 flex item-center justify-center bg-primary h-6 rounded'>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md'>
          ...
          <input type="number" min={0} value={showPrice}
      ...
      {Object.keys(dateTimeSelection).length > 0 && (
        <div className='mt-6'>
          <h2 className='mb-2'>Slected Date-Time</h2>
          <ul className='space-y-3'>
            {Object.entries(dateTimeSelection).map(([date, times]) => (
              <li key={date}>
                <div className='font-medium'>{date}</div>
                <div>{times.map((time) => (
                  <div key={time} className='border border-primary px-2 py-1 w-20 flex items-center rounded'>
                    <span>{time}</span>
